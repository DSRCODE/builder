...
import { useLaborEntry, useUpdateLaborEntry } from "@/hooks/useLaborEntry";
...
interface EditLaborEntryModalProps {
  ...
  laborEntryId: number | null;
}

export function EditLaborEntryModal({
  open,
  onOpenChange,
  laborEntryId,
}: EditLaborEntryModalProps) {
  ...
  // Populate form when labor entry data loads
  useEffect(() => {
    if (laborEntry) {
      const entryDate = new Date(laborEntry.date);
      setDate(entryDate);

      setFormData({
        site_id: laborEntry.site_id.toString(),
        date: format(entryDate, "yyyy-MM-dd"),
        masons: laborEntry.masons.toString(),
        helpers: laborEntry.helpers.toString(),
        lady_helpers: laborEntry.lady_helpers.toString(),
        manual_total_wage: laborEntry.manual_total_wage || "",
        override_comment: laborEntry.override_comment || "",
      });
    }
  }, [laborEntry]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!laborEntryId) {
      toast.error(`${t("labor.form_error_editing_failed")}`);
      return;
    }

    // Validation
    if (!formData.site_id || !formData.date) {
      toast.error(`${t("labor.form_failed_error")}`);
      return;
    }

    try {
      await updateLaborEntry.mutateAsync({ id: laborEntryId, data: formData });

      // Close modal on success
      onOpenChange(false);
    } catch (error) {
      console.error(`${t("labor.failed_to_update_labor_entry")}`, error);
    }
  };
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{t("labor.edit_labor_entry")}</DialogTitle>
            <DialogDescription>
              {t("labor.loading_labor_entry")}
            </DialogDescription>
          </DialogHeader>
  ...
  if (entryError) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{t("labor.edit_entry_title")}</DialogTitle>
            <DialogDescription>
              {t("labor.edit_entry_description")}
            </DialogDescription>
          </DialogHeader>

          <Alert variant="destructive">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>{entryError.message}</AlertDescription>
          </Alert>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {sitesLoading ? (
              ...
              <Alert variant="destructive">
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  {t("labor.site_details_not_avaiable")} {sitesError.message}
                </AlertDescription>
              </Alert>
            ) : (
              <Select
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={
                updateLaborEntry.isPending || sitesLoading || entryLoading
              }
            >
              {updateLaborEntry.isPending && (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              )}
              {updateLaborEntry.isPending
                ? `${t("labor.buttons.updating")}`
