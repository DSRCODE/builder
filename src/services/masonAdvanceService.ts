...
import { MasonAdvance, MasonAdvancesResponse, MasonAdvanceFormData, MasonAdvanceApiResponse } from '@/types/masonAdvance';
...
export const masonAdvanceService = {
  ...
  createMasonAdvance: async (masonAdvance: MasonAdvanceFormData): Promise<MasonAdvanceApiResponse> => {
    try {
  ...
  updateMasonAdvance: async (id: number, masonAdvance: MasonAdvanceFormData): Promise<MasonAdvanceApiResponse> => {
    try {
      ...
      if (error.response?.status === 422 || error.response?.status === 405) {
        try {
          const response = await api.post(`/mason-advances/${id}`, masonAdvance);
          const data = response.data;
